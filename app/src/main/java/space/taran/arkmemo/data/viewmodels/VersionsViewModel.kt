...
class VersionsViewModel @Inject constructor() : ViewModel() {
    ...
    @Inject
    lateinit var versionsRepo: VersionsRepository
        ...
        viewModelScope.launch(iODispatcher) {
            ...
            when (versionsRepo.deleteNoteFromVersion(note, version.value!!.meta!!.rootResourceId)) {
                ...
                CODES_DELETING_NOTE.SUCCESS.code -> {
                    val versionWithoutNote = Version(
                        versionsRepo.getVersionContent(version.value!!.meta!!.rootResourceId),
                        version.value!!.meta
                    )
                    version.value = versionWithoutNote
                    textNotes.value =
                        textNotesRepo.getAllNotesFromVersion(version.value!!.meta!!.rootResourceId)
                }
            }
        }
    }
